...
with open('new.csv', 'wb') as newcsv:
    ...
    fieldnames = ['tweet_id','owner_name', 'text','original_author', 'time','date', 'in_reply_to_screen_name',
    ...
    for row in reader:
        ...
        if keywords!=[]:
           while i <(len(keywords)-1):
               i+=1
               # Liste alle Hashtags (distinct) herstellen
               if keywords[i] not in all_keywords:
                   all_keywords.append(keywords[i])
                #Daten in die Datei schreiben, falls Hashtag gefunden wurde
               writer.writerow({'tweet_id':count, 'owner_name': row['handle'], 'text': row['text'],
                             'original_author': row['original_author'],
                             'time': zeit[0],'date':datum[0], 'in_reply_to_screen_name': row['in_reply_to_screen_name'],
            ...
            writer.writerow({'tweet_id':count, 'owner_name': row['handle'], 'text': row['text'],
                             'original_author': row['original_author'],'time': zeit[0],'date':datum[0], 'in_reply_to_screen_name': row['in_reply_to_screen_name'],
